...
class TypescriptHelper(Helper):
    ...
    def add_required_import(required_import: set, import_type, class_name, base_class_name):
        for typename in re.split('[\\[\\]]', import_type):
            if typename and typename not in ['List', 'Uint8Array', 'bigint']:
    ...
    def get_read_method_name(self, size, var_name):
        ...
        if size == 8:
            return 'GeneratorUtils.bufferToBigInt(Uint8Array.from({0}))'.format(var_name)
        if size == 4:
    ...
    def get_serialize_method_name(self, size):
        if isinstance(size, str) or size > 8:
            return ''
        if size == 8:
            return 'GeneratorUtils.bigIntToBuffer'
        if size == 4:
    ...
    def get_empty_value(self, size):
        if isinstance(size, str) or size > 8:
            return 'Buffer.alloc(' + str(size) + ')'
        if size == 8:
            return 'BigInt(0)'
        if size < 8:
            return '0'
    ...
    def get_immutable_attribute_name(self, immutable_attribute):
        #  https://github.com/symbol/catbuffer-schemas/issues/3 workaround
        if immutable_attribute.attribute_name == 'entityType':
            return 'type'
        return immutable_attribute.attribute_name

    def get_immutable_attribute(self, attribute_name, immutable_attributes):
        return next((immutable_attribute for immutable_attribute in immutable_attributes
                     if self.get_immutable_attribute_name(immutable_attribute) == attribute_name), None)
