...
import { ${a} } from './${a}';
% endfor
% if generator.generated_base_class_name is not None:
import { ${str(generator.generated_base_class_name)}Params } from './${str(generator.generated_base_class_name)}'
% endif

/**
*  Interface to create instances of ${generator.generated_class_name}.
*/
export interface ${generator.generated_class_name}Params ${(' extends ' + str(generator.generated_base_class_name + 'Params')) if generator.generated_base_class_name is not None else ''}{
% for a in [a for a in generator.all_constructor_params if not a.attribute_is_reserved and not a.attribute_is_aggregate and not a.attribute_is_super]:
    /** ${helper.capitalize_first_character(a.attribute_comment)}. **/
    ${a.attribute_name + ('?' if a.attribute_is_conditional else '')}: ${a.attribute_var_type };
% endfor
}
...
% for a in generator.immutable_attributes:
    ...
    public static readonly ${helper.snake_case(a.attribute_name).upper()} = ${a.attribute_var_type}.${helper.snake_case(generator.name).upper()};
    % else:
    public static readonly ${helper.snake_case(a.attribute_name).upper()} = ${a.attribute_value};
    % endif
% endfor
...
% for a in [a for a in generator.attributes if not a.attribute_is_super and not a.attribute_is_inline and not a.kind == helper.AttributeKind.SIZE_FIELD and not a.attribute_is_reserved and a.attribute_name != 'size']:
    /** ${helper.capitalize_first_character(a.attribute_comment)}. **/
    public readonly ${a.attribute_name}${('?:' if a.attribute_is_conditional else ':')} ${a.attribute_var_type};
...
<%def name="renderCondition(a)" filter="trim">
    ...
    <%
        ...
        constructor_params_CSV = ', '.join([ str(a.attribute_name) for a in constructor_params if a.attribute_condition_value == None and not a.attribute_is_aggregate and not a.attribute_is_reserved and not a.attribute_name == 'size'])
...
% endfor
    */
    public constructor({${constructor_params_CSV}}: ${generator.generated_class_name}Params) {
    ...
    % endfor
    % for a in generator.immutable_attributes:
        if (${helper.get_immutable_attribute_name(a)} !== ${generator.generated_class_name}.${helper.snake_case(a.attribute_name).upper()})
           throw new Error('The provided ${helper.get_immutable_attribute_name(a)} value ' + ${helper.get_immutable_attribute_name(a)} + ' is invalid. Expected value is ' + ${generator.generated_class_name}.${helper.snake_case(a.attribute_name).upper()});
    % endfor
    }
...
##       TODO. Size 2 is hardcoded here. Improve!
   ...
   % elif a.kind == helper.AttributeKind.ARRAY:
        ...
        byteArray.splice(0, ${a.attribute_name}.reduce((sum, c) => sum + c.size, 0));
    ...
    % elif a.kind == helper.AttributeKind.CUSTOM:
        ...
        byteArray.splice(0, ${a.attribute_name}.size);
    ...
    % elif a.kind == helper.AttributeKind.FILL_ARRAY:
        ...
        byteArray.splice(0, ${a.attribute_name}.reduce((sum, c) => sum + GeneratorUtils.getSizeWithPadding(c.size, ${helper.resolve_alignment(a)}), 0));
...
% for possible_constructor_params in generator.constructor_attributes:
    <%
        constructor_params = [a for a in possible_constructor_params if  helper.get_immutable_attribute(a.attribute_name, generator.immutable_attributes) is None and a.attribute_condition_value is None and a.attribute_condition_provide and not a.attribute_is_reserved and not a.attribute_is_aggregate]
        ...
        constructor_arguments_CSV = ', '.join([a.attribute_name + ': ' + str(a.attribute_name)
        for a in possible_constructor_params if not a.attribute_is_aggregate and not a.attribute_is_reserved and not a.attribute_name == 'size' and (a.attribute_condition_value is not None or a.attribute_condition_provide)])
    %>
    /**
...
% endfor
    ...
    % for a in generator.immutable_attributes:
        const ${helper.get_immutable_attribute_name(a)} = ${generator.generated_class_name}.${helper.snake_case(a.attribute_name).upper()};
    % endfor
...
## GETTERS:
% for a in [a for a in generator.attributes if not a.attribute_is_super and not a.attribute_is_reserved and a.attribute_is_inline  and not a.attribute_is_aggregate and not a.kind == helper.AttributeKind.SIZE_FIELD and (not a.attribute_is_reserved or not a.attribute_is_inline) and a.attribute_name != 'size']:
    /**
     * Gets ${a.attribute_comment}.
     *
     * @return ${helper.capitalize_first_character(a.attribute_comment)}.
     */
    public get ${a.attribute_name if a.attribute_name != 'size' else 'StreamSize'}(): ${a.attribute_var_type}
    % if a.attribute_is_conditional:
        | undefined
    % endif
   {
    % if a.attribute_is_inline:
        return this.${a.attribute_aggregate_attribute_name}.${a.attribute_name};
    % else:
...
%>\
   ...
   % elif a.kind == helper.AttributeKind.ARRAY or a.kind == helper.AttributeKind.VAR_ARRAY or a.kind == helper.AttributeKind.FILL_ARRAY:
        size += ${this_attribute_name}.reduce((sum, c) => sum + GeneratorUtils.getSizeWithPadding(c.size, ${helper.resolve_alignment(a)}), 0);  // ${a.attribute_name}
    % elif a.kind == helper.AttributeKind.FLAGS:
        size += ${a.attribute_size};  // ${a.attribute_name}
    % elif a.kind == helper.AttributeKind.CUSTOM and a.attribute_base_type == 'enum':
        size += ${a.attribute_size}; // ${a.attribute_name}
    % else:
        size += ${this_attribute_name}.size;  // ${a.attribute_name}
    % endif
</%def>\
    ...
    public get size(): number {
        let size = ${'super.size' if generator.base_class_name is not None else '0'};
...
% if generator.base_class_name in ['Transaction', 'EmbeddedTransaction']:
     ...
     * @return Body builder.
     */
    public get body(): ${generator.body_class_name}Builder  {
...
%>\
    ...
    % elif a.kind == helper.AttributeKind.SIZE_FIELD and 'disposition' in a.parent_attribute and a.parent_attribute['disposition'] == 'var':
            const ${a.attribute_name} = this.${a.parent_attribute['name']}.reduce((sum, c) => sum + GeneratorUtils.getSizeWithPadding(c.size, ${helper.resolve_alignment(a)}), 0);
