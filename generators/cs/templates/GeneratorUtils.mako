    ...
    public class GeneratorUtils {
        ...
        */
        public static List<T> LoadFromBinaryArrayRemaining<T>(
            ...
            {
                var ms = new MemoryStream();
                var msStartPosition = stream.BaseStream.Position;
                stream.BaseStream.CopyTo(ms);
                var ms2 = new MemoryStream();
                var bw = new BinaryWriter(ms2);
                var br = new BinaryReader(ms2);
                bw.Write(ms.ToArray(), 0, payloadSize);
                var ms2StartPosition = ms2.Position;
                ms2.Position = 0;
                while (ms2.Position < ms2.Length){
                    var entity = builder(br);
                    entities.Add(entity);
                ...
                stream.BaseStream.Position = msStartPosition + ms2StartPosition;
            }
            return entities;
        }
