...
class AddressKeyValueSetBuilder:
    ...
    @classmethod
    def load_from_binary(cls, payload: bytes) -> AddressKeyValueSetBuilder:
        ...
        for _ in range(keyValueCount):
            item = AddressKeyValueBuilder.load_from_binary(bytes_)
            keys.append(item)
            bytes_ = bytes_[item.get_size():]
        return AddressKeyValueSetBuilder(keys)

    def get_keys(self) -> List[AddressKeyValueBuilder]:
