...
**/
public class AccountOperationRestrictionTransactionBodyBuilder implements Serializer {
    ...
    protected AccountOperationRestrictionTransactionBodyBuilder(DataInputStream stream) {
        try {
            ...
            this.restrictionAdditions = GeneratorUtils.loadFromBinaryArray(EntityTypeDto::loadFromBinary, stream, restrictionAdditionsCount, 0);
            this.restrictionDeletions = GeneratorUtils.loadFromBinaryArray(EntityTypeDto::loadFromBinary, stream, restrictionDeletionsCount, 0);
        } catch (Exception e) {
